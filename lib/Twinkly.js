...
class Twinkly {
    ...
    playSavedMovie(movieName) {
        const fs = require('fs');
        const path = require('path');
        const moviePath = path.join(__dirname, '../re-movies', movieName + '.bin');
        ...
        return this.ensureDeviceInfo()
            ...
            .then(data => {
                this.requestService.postOctet("led/movie/full", data)
                return data
            })
            .then((data) => this.requestService.postJson("led/movie/config", {
                frame_delay: 100, // Assuming a default frame delay
                frames_number: data.length / this.ledCount, // Assuming each frame is ledCount bytes
                leds_number: this.ledCount
            }));
    }

    setMovingGradient(colors, delay = 100, frameCount = 100, addTwinkle = false) {
        return this.ensureDeviceInfo()
            .then(() =>
                this.setMovie(
                    Movie.movingGradient(this.ledCount, frameCount, delay, colors, addTwinkle)
                )
            );
    }
    ...
    setMovie(movie) {
        return this.ensureDeviceInfo()
            ...
            .then(() => this.requestService.postOctet("led/movie/full", movie.getArraySpiral(this.ledProfile, 20, 20)))
